...
for d =1:length(Demos)
    ...
    for i = 1:numChuncks
        TNew = cat(1, TNew, {t(inds(i):inds(i+1))});
        DemosNew = cat(1, DemosNew, {demo(:, inds(i):inds(i+1))});
        DemosdNew = cat(1, DemosdNew, {demod(:, inds(i):inds(i+1))});
        DemosddNew = cat(1, DemosddNew, {demodd(:, inds(i):inds(i+1))});
